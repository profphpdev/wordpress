jQuery(document).ready(function() {
    ...
    var table = jQuery('#example').dataTable({
        ...
        "iDisplayLength": 10,
        "aaSorting":[[2, "asc"]],
        "sAjaxSource":ajax_object.ajaxurl+'?thema='+jQuery('.value1').val()+'&naam='+jQuery('.value2').val(),
        "sServerMethod": "POST",
        "fnServerParams": function ( aoData ) {
            aoData.push( { name: "action", value: "getSearchList" } );
        },
    ...
    jQuery("#example tbody tr>td:first-child").live({
        mouseenter:
            function (ev) {
                ...
                jQuery('#popup_hover').css({'left':500,'top':ev.pageY}); 
                 request = jQuery.ajax({
                  type: "POST",
                  url: ajax_object.ajaxurl2,
                  data: { name: id, action: 'updateJournalItem', type: 'hover'},
...
//              type: "POST",
//              url: ajax_object.ajaxurl2,
//              data: { name: obj_id, action: 'updateJournalItem', type: 'click'},
//              success:function(data)
//              {
