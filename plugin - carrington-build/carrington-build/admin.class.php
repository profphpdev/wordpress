...
class cfct_build_admin extends cfct_build_common {
	...
	public function row_chooser() {
                $row_tabs = array('All');
                $row_tab_contents = array();
                $row_types_html =   '<div id="tabs-module-content">'.
                                    '<div class="view_container">';
                foreach ($this->template->rows as $id => $row){
                    $row_tab = $row->get_tab();
                    if(!in_array($row_tab, $row_tabs)){
                        array_push($row_tabs, $row_tab);
                    }
                    $row_html = '
				<li class="cfct-il-item" id="cfct-row-type-'.$id.'">
					<a class="cfct-il-a" href="#cfct-row-type-'.$id.'" row_group="'.$row->get_group().'" rel="'.$id.'" title="'.esc_attr($row->get_desc()).'">
				...
				';
                    $row_tab_contents[$row_tab] .= $row_html;
                    $row_tab_contents['All'] .= $row_html;
                }
                sort($row_tabs);
                $row_tabs_html .=   '<div id="tabs_container">'.
                                    '<ul class="tabs">';
                foreach($row_tabs as $row_tab){
                    $row_tabs_html .=   '<li>'.
                                        '<a href="#row_content_'.strtolower($row_tab).'" class="tab">'.$row_tab.'</a>'.
                                        '</li>';
                    $row_types_html .=  '<div id="row_content_'.strtolower($row_tab).'" class="tab_view">'.
                                        '<ul class="cfct-il cfct-il-mini il-hover-titles cfct-clearfix cfct-rc-row-type-list">'.
                                        $row_tab_contents[$row_tab].
                                        '</ul>'.
                                        '</div>';
                }
                $row_tabs_html .=   '</ul>'.
		...
		$html = '
			<div id="cfct-sortables-add-container">
				<div id="cfct-loading-row" style="display: none;"></div>
                                <div id="cfct-wrapper-select-new-row">
                                    <div id="cfct-container-select-new-row">
                                        <div id="cfct-select-new-row">
                                                <div class="cfct-popup-anchored cfct-popup-anchored-bottom">
                                                        <div class="cfct-popup">
                                                                ...
                                                                <div id="tabs_horizontal" class="cfct-popup-content">'.
                                                                        $row_tabs_html.
                                                                        $row_types_html.
                                                                '   <div class="clear"></div>
                                                                    </div><!-- /cfct-popup-content -->
                                        ...
                                        </div><!--/ccfct-container-select-new-row-->
                                </div><!--/cfct-wrapper-select-new-row-->
	...
	public function ajax_insert_template($args) {
        ...
        public function ajax_settings_row($args) {
            ...
            if ($this->template->set_template($post_data['template'])) {
                ...
                $result = $this->template->row_settings($row['settings'], $row['guid']);
                $ret = new cfct_message(array(
				'success' => true,
				'html' => $result['html'],
				//'message' => __((isset($result['message']) ? $result['message'] : 'operation successful'), 'carrington-build')
                    ...
                    throw new cfct_template_exception(__('Could not load raw settings.','carrington-build'));
            }
            do_action('cfct-ajax-settings-row', $this, $args['post_id']);
            return $ret;
        }
	/**
	...
	public function ajax_edit_module($args) {
        ...
        public function ajax_save_row_settings($args){
            $post_data = $this->get_postmeta($args['post_id']);
            if($this->template->set_template($post_data['template'])){
                $row = $this->template->get_row_data($args['row_id']);
                parse_str($args['data'], $data);
                if(is_array($data) && !empty($data) && isset($post_data["template"]["rows"][$args["row_id"]]["settings"])){
                    $arr_base='$post_data["template"]["rows"][$args["row_id"]]["settings"]';
                    foreach($data as $data_key => $data_val){
                        $single_data_arr = explode('-', $data_key);
                        $code = $arr_base;
                        foreach($single_data_arr as $single_data_val){
                            $code .= '["' . $single_data_val . '"]';
                        }
                        $code .= '["default"]=$data_val;';
                        eval($code);
                    }
                }
            }
            ...
            $ret = new cfct_message(array(
					'success' => true,
					'message' => __('edit successful for', 'carrington-build').' row_id: '.$args['row_id']
            ));
            return $ret;
        }
